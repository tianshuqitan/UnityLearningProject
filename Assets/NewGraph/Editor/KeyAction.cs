    ...
    public class KeyAction
    {
        ...
        private void CreateDisplayName()
        {
            void CreateDisplayCodeFromHandler(Handler handler, ref string displayCode)
            {
                ...
                {
                    if (ShortcutHandler.specialKeyDisplayNameLookup.ContainsKey(specialKey))
                    {
                        displayCode += ShortcutHandler.specialKeyDisplayNameLookup[specialKey] + "+";
                    }
                }
            }
        ...
        private bool AreChecksValid(EventModifiers eventModifiers, SpecialKey[] checks)
        {
            ...
            {
                if (!ShortcutHandler.specialKeyChecks[checks[i]](eventModifiers))
                {
                    return false;
                }
            }
