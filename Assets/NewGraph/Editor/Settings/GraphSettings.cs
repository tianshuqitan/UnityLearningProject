...
namespace NewGraph
{
    ...
    public class GraphSettings
    {
        ...
        public const string lastOpenedDirectoryPrefsKey = nameof(NewGraph) + "." + nameof(GraphSettingsAsset) + "." +
                                                          nameof(lastOpenedDirectoryPrefsKey);

        public const string isInspectorVisiblePrefsKey = nameof(NewGraph) + "." + nameof(GraphSettingsAsset) + "." +
        ...
        public static LastGraphInfo LastOpenedGraphInfo
        {
            get
            {
                var lastOpenedGraphGUID = EditorPrefs.GetString(lastOpenedGraphKey, null);
                var lastOpenedGraphType = EditorPrefs.GetString(lastOpenedGraphTypeKey, null);
                ...
                var lastGraphInfo = new LastGraphInfo
                {
                    GUID = lastOpenedGraphGUID,
                    graphType = System.Type.GetType(lastOpenedGraphType)
                };
        ...
        public static Image HomeButtonIcon => new Image()
            { image = EditorGUIUtility.IconContent(Settings.homeButtonIcon).image };

        [SerializeField] private string createButtonIcon = "d_Toolbar Plus";

        public static Image CreateButtonIcon => new Image()
            { image = EditorGUIUtility.IconContent(Settings.createButtonIcon).image };

        [SerializeField] private string loadButtonIcon = "d_FolderOpened Icon";

        public static Image LoadButtonIcon => new Image()
            { image = EditorGUIUtility.IconContent(Settings.loadButtonIcon).image };

        [SerializeField] private string resetButtonIcon = "Refresh@2x";

        public static Image ResetButtonIcon => new Image()
            { image = EditorGUIUtility.IconContent(Settings.resetButtonIcon).image };
        ...
        [Range(150, 400)] public int nodeWidth = 300;
                ...
                if (handleBarsPartial == null)
                {
                    handleBarsPartial =
                        AssetDatabase.LoadAssetAtPath<VisualTreeAsset>(
                            AssetDatabase.GUIDToAssetPath(Settings.handleBarsPartialIdentifier));
                }
        ...
        public static VisualTreeAsset graphDocument =>
            AssetDatabase.LoadAssetAtPath<VisualTreeAsset>(
                AssetDatabase.GUIDToAssetPath(Settings.graphDocumentIdentifier));

        [SerializeField] private string graphStylesheetVariablesIdentifier = "a22fe40a267e5824dbde2cea493f40e4";

        public static StyleSheet graphStylesheetVariables =>
            AssetDatabase.LoadAssetAtPath<StyleSheet>(
                AssetDatabase.GUIDToAssetPath(Settings.graphStylesheetVariablesIdentifier));
        ...
        public static StyleSheet graphStylesheet =>
            AssetDatabase.LoadAssetAtPath<StyleSheet>(
                AssetDatabase.GUIDToAssetPath(Settings.graphStylesheetIdentifier));

        [SerializeField] private string settingsStylesheetIdentifier = "7d7e4dcd278879849b62a784de9bdc3c";

        public static StyleSheet settingsStylesheet =>
            AssetDatabase.LoadAssetAtPath<StyleSheet>(
                AssetDatabase.GUIDToAssetPath(Settings.settingsStylesheetIdentifier));
